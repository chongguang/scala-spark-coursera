...
object WikipediaRanking {
  ...
  rdd.aggregate(0)(
    (acc, article) => if (article.text.split(" ").contains(lang)) 1 + acc else acc,
    (nb1, nb2) => nb1 + nb2
  )
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    val langToArticlesRDD =
      rdd.flatMap(article => langs.filter(lang => article.text.split(" ").contains(lang))
        .map(language => (language, article)))
    langToArticlesRDD.groupByKey
  }
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] =
    index.mapValues(l=>l.size).sortBy(_._2).collect().toList.reverse
