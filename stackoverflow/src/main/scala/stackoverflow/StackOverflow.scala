...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(Int, Iterable[(Posting, Posting)])] = {
    val questions: RDD[(Int, Posting)] = postings.filter(p => p.postingType == 1).map(q => (q.id, q))
    val answers: RDD[(Int, Posting)] = postings.filter(p => p.postingType == 2 && p.parentId.isDefined).map(a => (a.parentId.get, a))
    val joined: RDD[(Int, (Posting, Posting))] = questions.join(answers)
    joined.groupByKey()
  }
  ...
  def vectorPostings(scored: RDD[(Posting, Int)]): RDD[(Int, Int)] = {
    ...
    scored.map( t => (firstLangInTag(t._1.tags, langs).get * langSpread, t._2)).cache()
  }
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    val newCenters = closest.groupByKey().mapValues(averageVectors)
    val newCentersMap = newCenters.collect().toMap

    val oldCenters = means.zipWithIndex.map(t => t.swap)
    val newMeans = oldCenters.map(c => newCentersMap.getOrElse(c._1, c._2))
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val grouped: Map[Int, Int] = vs
        .map(_._1 / langSpread) // recompute original index by dividing by the langSpread
